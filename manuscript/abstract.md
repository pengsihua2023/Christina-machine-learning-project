...
0.835 ± 0.060; permutation p = 0.0099 against a null of 0.501 ± 0.049). Sampling
...
(ExtraTrees AUC 0.858 vs SVM-RBF 0.835, Wilcoxon p = 0.041, not surviving correction for 15 comparisons), and three of the top
...
unambiguous — a radial basis kernel outperformed a linear one on 24 of 25 folds
(ΔAUC = +0.080, p < 0.0001) — indicating genuine nonlinear structure, consistent
...
generalisation failed (AUC 0.56 ± 0.28 across four host species).
...
**结果。** 基于菌群的分类显著优于随机（SVM-RBF AUC 0.835 ± 0.060；置换检验
p = 0.0099，零分布 0.501 ± 0.049）。采样月份是强混杂因子：仅协变量（季节、地点、
物种、性别）即达 AUC 0.881，且菌群本身预测"是否采样于 11–12 月"的 AUC 为 0.777。
由于季节是混杂因子而非感兴趣的暴露，主估计采用分层法，
模型中只保留菌群特征：在单一月份内部，AUC 从 10 月的 0.734（准确率 0.676，特异度
0.533）到 7 月的 0.965（准确率 0.949，特异度 0.929），7 月与 8 月合并为 0.959。由于
11 月与 12 月全部为阳性，仅 165/260 个样本可进入分层分析。若改用协变量调整，菌群带来
的 AUC 增量为 0.043 至 0.105，取决于所选模型。模型选择无法由判别能力决定：两个领跑
模型在统计上无法区分（ExtraTrees AUC 0.858 对 SVM-RBF 0.835，Wilcoxon p = 0.041，未通过 15 次比较的校正），
且前四名中有三个构成互相无法区分的一组（两两 p = 0.360–0.609），因此领先者之间不存在
一致的排名。相反，一次受控的核函数消融给出了明确
结论——径向基核在 25 折中 24 折优于线性核（ΔAUC = +0.080，p < 0.0001）——
...
其增量价值有限。本研究结论仅适用
于该队列；跨研究泛化失败（四个宿主物种间 AUC 0.56 ± 0.28）。
