...
STALE = [
    ...
    # 0.908 曾是鸭队列的旧加权估计。火鸡队列的纯笼效应恰好也是 0.908（见
    # results/turkey_strain_cage.json），是完全不同的量，因此这条规则必须限定
    # 在「加权/weighted」语境里，不能裸匹配数字。
    (r"(加权|weighted)[^。.]{0,20}0\.908|0\.908[^。.]{0,20}(加权|weighted)",
     "0.908 作为鸭队列的旧加权值已废弃，现为 0.894 且不作主报告"
     "（火鸡纯笼效应的 0.908 是另一个量，不受此限）"),
